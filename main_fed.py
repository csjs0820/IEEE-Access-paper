...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        ...
        if args.iid == 'iid':
            ...
            dict_users = hybrid1(dataset_train, args.num_users, 0.06)
    ...
    wd = [[] for i in range(args.epochs)]
    avg_wd = [[] for i in range(args.epochs)]
    ...
    for iter in range(args.epochs):
        ...
        idxs_users = {0,1,40,60}
        #idxs_users = np.arange(args.num_users)
        #idxs_users = random_client
        ...
        for client in idxs_users:
            wd[iter].append(float(weight_divergence(iid_w,w_locals[client],client)))
            ww_locals.append(w_locals[client])

        #print("client 별 weight :",wd[iter])

        #avg_wd[iter] = float(sum(wd[iter]))/float(len(wd[iter]))

        avg_wd[iter].append((sum(wd[iter][0:2]))/(len(wd[iter][0:2])))
        avg_wd[iter].append((sum(wd[iter][1:])) /(len(wd[iter][1:])))

        selected_total_size = sum(len(dict_users[idx]) for idx in idxs_users)
        mixing_coefficients = [len(dict_users[idx]) / selected_total_size for idx in idxs_users]
        ...
        if len(final_client) < int(alpha*args.num_users):
            ...
            if len(candidate_client) != 0:
                ...
                for i in range(len(candidate_client)):
                    ...
                    dk[1][i] = weight_divergence(iid_w,w_locals[candidate_client[i]])
                for j in range(len(random_client)):
                    dj[0][j] = random_client[j]
                    dj[1][j] = weight_divergence(iid_w,w_locals[random_client[j]])
                dj = np.array(dj)
                dj = dj[:,dj[1,:].argsort()]
                random_client = dj[0]
        ...
        print('idd_model Round {:3d}, loss {:.3f} Test accuarcy {:.3f}'.format(iter,loss_w, acc_test2))
        # print loss
    ...
    print("Round 당 client weight divergence : ",wd)
    ...
    #plt.savefig('./{}/loss_{}_{}_{}_C{}_iid{}.png'.format(args.iid,args.dataset, args.model, args.epochs, args.frac, args.iid))
    plt.savefig('./save/{}/all_iid_client_loss.png'.format(args.iid))

    # plot acc curve
    plt.figure()
    plt.plot(range(len(test_acc)), test_acc)
    plt.title('all iid client test acc')
    plt.ylabel('test_acc')
    #plt.savefig('./{}/acc_fed_{}_{}_{}_C{}_iid{}.png'.format(args.iid,args.dataset, args.model, args.epochs, args.frac, args.iid))
    plt.savefig('./save/{}/all_iid_client_test_acc.png'.format(args.iid))
    '''
    # plot weight divergence curve
    iid_wd = []
    non_iid_wd = []
    for i in range(len(avg_wd)):
        iid_wd.append(avg_wd[i][0])
        non_iid_wd.append(avg_wd[i][1])

    plt.figure()
    x = range(len(avg_wd))
    plt.plot(x,iid_wd)
    plt.plot(x,non_iid_wd)
    plt.title('iid weight divergnece vs non iid weight divergence')
    plt.ylabel('weight divergence')
    plt.savefig('./save/{}/iid vs non iid weight divergence.png'.format(args.iid))
